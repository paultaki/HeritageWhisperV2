...
export function extractEntities(transcript: string): ExtractedEntities {
  ...
  console.log('[Entity Extraction] Processing transcript:', transcript.substring(0, 100) + '...');
  
  // Extract people - combination of proper nouns and role nouns
  const people = new Set<string>();
  const excludeWords = new Set([
    'The', 'I', 'We', 'They', 'And', 'But', 'So', 'Then', 'When', 'Where', 
    'What', 'How', 'Why', 'This', 'That', 'These', 'Those', 'There', 'Here',
    'Now', 'Just', 'Only', 'Even', 'Still', 'Yet', 'Already', 'Before', 'After'
  ]);
  
  // Pattern 1: Role nouns WITH articles (the girl, my friend, Coach, etc.)
  const roleNouns = [
    'coach', 'teacher', 'instructor', 'mentor', 'boss', 'manager',
    'friend', 'boyfriend', 'girlfriend', 'partner', 'spouse',
    'father', 'mother', 'dad', 'mom', 'parent', 'son', 'daughter', 'child', 'kid',
    'brother', 'sister', 'sibling', 'cousin', 'uncle', 'aunt', 'nephew', 'niece',
    'grandfather', 'grandmother', 'grandpa', 'grandma', 'grandchild', 'grandson', 'granddaughter',
    'husband', 'wife', 'neighbor', 'neighbour', 'colleague', 'coworker',
    'doctor', 'nurse', 'lawyer', 'officer', 'captain', 'sergeant',
  ...
  const standaloneTitles = ['Coach', 'Teacher', 'Doctor', 'Professor', 'Captain', 'Sergeant', 'Officer', 'Pastor', 'Father', 'Mother', 'Boss'];
  ...
  const nameWithVerbPattern = /\b([A-Z][a-z]{2,}(?:\s+[A-Z][a-z]+)?)\s+(?:said|told|taught|showed|gave|asked|wanted|helped|loved|knew|met|called|kept|started|stopped|was|were|had)/g;
  ...
  const withFromPattern = /\b(?:with|from)\s+([A-Z][a-z]{2,}(?:\s+[A-Z][a-z]+)?)\b/g;
  ...
  for (const match of withFromMatches) {
    ...
    if (!excludeWords.has(name) && (!nextChar || nextChar === ' ' || nextChar === '.' || nextChar === ',')) {
      people.add(name);
    }
  }
  ...
  placePatterns.forEach((pattern, index) => {
    ...
    for (const match of matches) {
      ...
      if (place && !['The', 'I', 'We'].includes(place)) {
        ...
        if (index > 0) { // Skip first pattern (capitalized cities)
  ...
  objectPatterns.forEach(pattern => {
    ...
    for (const match of matches) {
      ...
      const commonWords = ['time', 'day', 'year', 'way', 'thing', 'lot', 'kind', 'sort', 'bit', 'place', 'moment'];
  ...
  const emotionWords = [
    ...
    'joyful', 'anxious', 'grateful', 'regretful', 'confused',
    'frustrated', 'surprised', 'shocked', 'delighted', 'worried'
  ];
...
const TEMPLATE_LIBRARY: Record<string, PromptTemplate> = {
  person_expansion: {
    ...
    patterns: [
      ...
      "What did {person} teach you?",
      "What's a specific memory you have of {person}?"
    ],
  ...
  temporal_sequence: {
    ...
    patterns: [
      "What happened right after that {temporal}?",
      "Tell me about the time before that.",
      "What led up to that {temporal}?"
    ],
...
export function generateTier1Templates(
  ...
  console.log(`[Tier 1 Template] Generated ${prompts.length} prompts:`, prompts.map(p => p.entity));
