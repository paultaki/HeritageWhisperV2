...
// Simplified timeline view for family members
export default function FamilyTimelinePage({
  ...
  const { data: storiesData, isLoading } = useQuery({
    queryKey: ['/api/family/stories', params.userId],
    queryFn: async () => {
      if (!session?.sessionToken) {
        throw new Error('No session token');
      }

      const response = await fetch(`/api/family/stories/${params.userId}`, {
        headers: {
  ...
  return (
    <FamilyGuard userId={params.userId}>
