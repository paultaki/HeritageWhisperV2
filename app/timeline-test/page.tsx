        ...
        <div className="flex items-center gap-3">
          ...
          {story.audioUrl && (
            ...
            >
              {isLoading ? (
                <Loader2
                  className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-heritage-orange"} animate-spin`}
                />
              ) : hasError ? (
                <AlertCircle className="w-4 h-4 text-red-500" />
              ) : isPlaying ? (
                <Pause
                  className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-heritage-orange"}`}
                />
              ) : (
                <Play
                  className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-heritage-orange"} ml-0.5`}
                />
              )}
            </button>
          )}
      ...
      <div className="retro-tv relative aspect-[3/2] mb-3 rounded-lg overflow-hidden bg-gray-100">
        ...
        {story.audioUrl && (
          ...
          >
            {isLoading ? (
              <Loader2
                className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-[#D4853A]"} animate-spin`}
              />
            ) : hasError ? (
              <AlertCircle className="w-4 h-4 text-red-500" />
            ) : isPlaying ? (
              <Pause
                className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-[#D4853A]"}`}
              />
            ) : (
              <Play
                className={`w-4 h-4 ${colorScheme === "retro" ? "text-[#5BB5B0]" : "text-[#D4853A]"} ml-0.5`}
              />
            )}
          </button>
        )}
          ...
          <button
            ...
            className={`
              ...
              transition-all duration-300 transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 ${
                currentColorScheme === "retro"
                  ? "focus-visible:ring-[#5BB5B0]"
                  : "focus-visible:ring-heritage-coral"
              }
              ${
                    ...
                    : currentColorScheme === "soft"
                      ...
                      : currentColorScheme === "cool"
                        ...
                        : currentColorScheme === "dark"
                          ...
                          : currentColorScheme === "retro"
                            ? "linear-gradient(to bottom, #D4654F, #5BB5B0)"
                            : "linear-gradient(to bottom, #D4853A, #F08466)",
            }}
          ></div>
