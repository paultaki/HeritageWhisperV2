...
export function DraggablePhoto({
  ...
  const handleDragStop = (e: any, data: any) => {
    ...
    if (nodeRef.current) {
      const container = nodeRef.current.closest('.book-v5-scroll');
      if (container) {
  ...
  return (
    <>
      ...
      <figure className="relative group" style={photoStyle}>
        ...
        {caption && (
          <figcaption className="text-[12px] mt-1 italic" style={{ color: COLORS.textMuted }}>
            {caption}
          </figcaption>
        )}
      </figure>
        ...
        >
          {(Object.keys(SNAP_ZONES) as unknown as SnapZone[]).map((zoneNum) => (
            <button
              ...
              style={{ 
                pointerEvents: 'auto',
                background: currentZone === zoneNum ? "var(--hw-primary-soft, #E0E5ED)" : "transparent",
                color: COLORS.text
              }}
            >
...
// Zone Indicators - using brand primary
    ...
    <div className="absolute inset-0 pointer-events-none">
      <div className="absolute inset-0 grid grid-cols-2 gap-4 p-8">
        ...
        <div
          ...
          style={{
            borderColor: currentZone === 1 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
            backgroundColor: currentZone === 1 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
            color: currentZone === 1 ? COLORS.text : COLORS.textMuted
          }}
        >
          Top Left
        </div>
        ...
        <div
          className={`border-2 border-dashed rounded-lg flex items-center justify-center text-sm font-medium transition-all`}
          style={{
            borderColor: currentZone === 2 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
            backgroundColor: currentZone === 2 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
            color: currentZone === 2 ? COLORS.text : COLORS.textMuted
          }}
        >
          Top Right
        </div>
        ...
        <div
          className={`border-2 border-dashed rounded-lg flex items-center justify-center text-sm font-medium transition-all`}
          style={{
            borderColor: currentZone === 3 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
            backgroundColor: currentZone === 3 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
            color: currentZone === 3 ? COLORS.text : COLORS.textMuted
          }}
        >
          Middle Left
        </div>
        ...
        <div
          className={`border-2 border-dashed rounded-lg flex items-center justify-center text-sm font-medium transition-all`}
          style={{
            borderColor: currentZone === 4 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
            backgroundColor: currentZone === 4 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
            color: currentZone === 4 ? COLORS.text : COLORS.textMuted
          }}
        >
          Middle Right
        </div>
      ...
      <div
        ...
        style={{
          borderColor: currentZone === 5 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
          backgroundColor: currentZone === 5 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
          color: currentZone === 5 ? COLORS.text : COLORS.textMuted
        }}
      >
        Center Full
      </div>
      ...
      <div
        ...
        style={{
          borderColor: currentZone === 6 ? COLORS.primary : 'rgba(32, 57, 84, 0.2)',
          backgroundColor: currentZone === 6 ? 'rgba(32, 57, 84, 0.1)' : 'transparent',
          color: currentZone === 6 ? COLORS.text : COLORS.textMuted
        }}
      >
        Bottom Center
      </div>
