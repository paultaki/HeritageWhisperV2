...
function BookV5PageContent() {
  ...
  const [isBookOpen, setIsBookOpen] = useState(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('book-v5-is-open') === 'true';
    }
    return false;
  });
  const [currentSpreadIndex, setCurrentSpreadIndex] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem('book-v5-spread-index');
  ...
  const [currentMobilePage, setCurrentMobilePage] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem('book-v5-mobile-page');
      return saved ? parseInt(saved, 10) : 0;
    }
    return 0;
  });
  ...
  useEffect(() => {
    sessionStorage.setItem('book-v5-is-open', isBookOpen.toString());
  }, [isBookOpen]);

  useEffect(() => {
    sessionStorage.setItem('book-v5-spread-index', currentSpreadIndex.toString());
  }, [currentSpreadIndex]);

  useEffect(() => {
    sessionStorage.setItem('book-v5-mobile-page', currentMobilePage.toString());
  }, [currentMobilePage]);
  ...
  // Debug logging
  console.log('[Book V5] Query setup:', {
    user: user?.id,
    activeContext,
    storytellerId,
    isContextLoading,
  ...
  const { data, isLoading, isFetching } = useQuery<{ stories: Story[] }>({
    ...
    queryFn: async () => {
      console.log('[Book V5] Fetching stories for storytellerId:', storytellerId);
      ...
      console.log('[Book V5] Fetch URL:', url);

      // Use apiRequest which handles authentication (JWT or family session token)
      const res = await apiRequest("GET", url);
      const result = await res.json();

      console.log('[Book V5] Fetched stories:', result?.stories?.length || 0);
      return result;
    },
  ...
  useEffect(() => {
    ...
    if (storyIndex !== -1) {
      ...
      // Clean up URL
      window.history.replaceState({}, '', '/book-v5');
    }
  ...
  if (isLoading || isFetching || !data) {
    return (
      <div className="book-v5-page flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4" style={{ borderColor: 'var(--hw-accent-gold, #CBA46A)' }}></div>
          <p style={{ color: 'var(--hw-page-bg, #F7F2EC)' }}>Loading your stories...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="book-v5-page flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          <h2 className="text-2xl font-semibold mb-4" style={{ color: 'var(--hw-page-bg, #F7F2EC)' }}>
            Your Book is Empty
          </h2>
          <p className="mb-6" style={{ color: 'var(--hw-border-strong, #B8AA9C)' }}>
            Start creating memories to see them appear here.
          </p>
          <button
            onClick={() => router.push("/")}
            className="px-6 py-3 text-white rounded-md transition-all"
            style={{ background: 'var(--hw-primary, #203954)' }}
          >
  ...
  if (!isBookOpen) {
    return (
      <>
        ...
        <div className="hidden lg:block book-v5-page overflow-hidden antialiased selection:bg-blue-500/30 selection:text-blue-100" style={{ color: 'var(--hw-page-bg, #F7F2EC)' }}>
      ...
      <div className="hidden lg:block book-v5-page overflow-hidden antialiased selection:bg-blue-500/30 selection:text-blue-100" style={{ color: 'var(--hw-page-bg, #F7F2EC)' }}>
        ...
        <div className="md:py-2 max-w-[1800px] mr-auto ml-auto pr-4 pb-12 pl-4" style={{ paddingTop: '52px' }}>
          ...
          <div className="relative mx-auto hidden lg:flex items-center justify-center" style={{ height: "calc(100dvh - 110px)" }}>
            ...
            >
              {/* Ambient shadow/vignette - using brand primary */}
              <div className="pointer-events-none absolute -inset-8 rounded-2xl" style={{ background: 'radial-gradient(1000px 400px at 50% 30%, rgba(32, 57, 84, 0.15) 0%, rgba(32, 57, 84, 0.08) 35%, transparent 70%)' }}></div>
              <div className="pointer-events-none absolute inset-0 rounded-2xl shadow-[0_40px_120px_-30px_rgba(0,0,0,0.5)]"></div>

              {/* Outer book cover/border - using brand primary */}
              <div
                aria-hidden="true"
                ...
                style={{
                  ...
                  left: "-30px",
                  right: "-30px",
                  background: "linear-gradient(180deg, #2d4a63 0%, #203954 50%, #1a3247 100%)",
                  boxShadow: "0 15px 50px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.06)"
                }}
              ></div>
              ...
              {/* Refined spine with book binding curve effect - using brand colors */}
              <div className="pointer-events-none absolute inset-y-0 left-1/2 -translate-x-1/2 w-12 md:w-14 lg:w-16">
                ...
                <div className="absolute inset-y-6 left-1/2 -translate-x-1/2 w-px opacity-70" style={{ background: 'var(--hw-primary, #203954)' }}></div>
                <div className="absolute inset-y-6 left-1/2 -translate-x-1/2 w-0.5 opacity-25 shadow-[inset_1px_0_0_rgba(0,0,0,0.45),0_0_0_1px_rgba(247,242,236,0.35)]"></div>
              </div>
              ...
              {currentSpreadIndex > 0 && (
                <button
                  ...
                  aria-label="Previous page"
                >
                  <div className="flex items-center justify-center w-10 h-10 rounded-full shadow-xl border group-hover:scale-110 transition-all" style={{ background: 'var(--hw-page-bg, #F7F2EC)', borderColor: 'var(--hw-border-subtle, #D2C9BD)' }}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ color: 'var(--hw-primary, #203954)' }}>
                ...
                <div className="px-6 py-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-full flex items-center justify-center shadow-sm transition active:scale-95"
                      style={{ background: 'var(--hw-section-bg, #EFE6DA)' }}
                      aria-label="Close"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: 'var(--hw-text-secondary, #4A4A4A)' }}>
                    ...
                    <>
                      {chapterGroups.map((group) => (
                        <section key={group.chapter.id} className="space-y-3">
                          {/* Group header */}
                          <h3 className="text-sm font-semibold tracking-tight uppercase" style={{ color: 'var(--hw-text-primary, #1F1F1F)' }}>
                            {group.chapter.title}
                          </h3>
                              ...
                              >
                                ...
                                <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-md ring-1 flex items-center justify-center" style={{ background: 'var(--hw-section-bg, #EFE6DA)', ringColor: 'var(--hw-border-subtle, #D2C9BD)' }}>
                                  {story.photoUrl ? (
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: 'var(--hw-text-muted, #8A8378)' }}>
                                ...
                                <div className="flex-1 min-w-0">
                                  <h4 className="font-medium truncate font-serif" style={{ color: 'var(--hw-text-primary, #1F1F1F)' }}>
                                    {story.title}
                                  </h4>
                                  <div className="flex items-center gap-1.5 text-xs mt-0.5" style={{ color: 'var(--hw-text-muted, #8A8378)' }}>
                    ...
                    <>
                      {decadeGroups.map(([decade, stories]) => (
                        <section key={decade} className="space-y-3">
                          {/* Group header */}
                          <h3 className="text-sm font-semibold tracking-tight" style={{ color: 'var(--hw-text-primary, #1F1F1F)' }}>
                            {decade}
                          </h3>
...
// Closed Book Cover Component - Heritage Premium Style with Brand Tokens
  ...
  return (
    <div className="relative mx-auto" style={{
      ...
      <div className="relative w-full h-full">
        {/* Book spine (left edge) - using brand primary */}
        <div
          className="absolute left-0 top-0 bottom-0 rounded-l-lg pointer-events-none"
          style={{
            width: '32px',
            background: "linear-gradient(90deg, #1a3247 0%, #203954 50%, #2d4a63 100%)",
        ...
        {/* Main cover button - using brand primary */}
        <button
          onClick={onOpen}
          className="relative w-full h-full cursor-pointer transition-transform hover:scale-[1.02] active:scale-[0.99]"
          style={{
            borderRadius: '0 12px 12px 0',
            background: "linear-gradient(145deg, #3a5d7a 0%, #203954 50%, #1a3247 100%)",
            boxShadow: "0 25px 80px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.1), inset 0 -1px 0 rgba(0,0,0,0.5), inset -3px 0 10px rgba(0,0,0,0.4)"
          }}
        ...
        >
          ...
          <div
            ...
            style={{
              borderRadius: '0 12px 12px 0',
              background: "radial-gradient(ellipse at 20% 20%, rgba(255,255,255,0.12) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(255,255,255,0.08) 0%, transparent 50%)",
            }}
          ></div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
            <h1
              ...
              style={{
                ...
                color: "var(--hw-page-bg, #F7F2EC)",
                textShadow: "0 3px 6px rgba(0,0,0,0.9), 0 -1px 2px rgba(255,255,255,0.06)",
                fontWeight: 600,
              }}
            >
              {userName}&apos;s<br />Story
            ...
            <div className="w-32 h-0.5 mb-6" style={{ background: "linear-gradient(90deg, transparent, var(--hw-accent-gold, #CBA46A), transparent)" }}></div>

            <p className="text-xl md:text-2xl font-medium" style={{ color: "var(--hw-accent-gold, #CBA46A)", textShadow: "0 2px 4px rgba(0,0,0,0.7)" }}>
              {storyCount} {storyCount === 1 ? 'memory' : 'memories'}
            </p>

            {/* Simplified cover text with pulse animation */}
            <p className="mt-12 text-sm font-medium book-v5-pulse" style={{ color: "var(--hw-page-bg, #F7F2EC)", opacity: 0.8 }}>
              Tap to open
            </p>
          </div>

          {/* Decorative embossed corner accents inside the border */}
          <div className="absolute top-14 left-14 w-8 h-8 border-l border-t rounded-tl" style={{ borderColor: "rgba(255,255,255,0.15)", filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.6))" }}></div>
          <div className="absolute top-14 right-14 w-8 h-8 border-r border-t rounded-tr" style={{ borderColor: "rgba(255,255,255,0.15)", filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.6))" }}></div>
          <div className="absolute bottom-14 left-14 w-8 h-8 border-l border-b rounded-bl" style={{ borderColor: "rgba(255,255,255,0.15)", filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.6))" }}></div>
          <div className="absolute bottom-14 right-14 w-8 h-8 border-r border-b rounded-br" style={{ borderColor: "rgba(255,255,255,0.15)", filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.6))" }}></div>
        </button>
      </div>
    </div>
  );
...
export default function BookV5Page() {
  return (
    <Suspense fallback={
      <div className="book-v5-page flex items-center justify-center">
        <div className="text-center">
