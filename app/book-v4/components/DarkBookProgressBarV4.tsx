...
interface DarkBookProgressBarV4Props {
  pages: BookPage[];
  currentPage: number;
  totalPages: number;
...
/**
 * DarkBookProgressBarV4 - Premium Progress Bar
 *
 * Enhanced version with sepia/gold theme colors from CSS variables.
 */
export default function DarkBookProgressBarV4({
  pages,
  currentPage,
  totalPages,
  onNavigateToPage,
        ...
        <div className="flex-1 w-full max-w-[calc(100%-90px)] md:max-w-[calc(100%-240px)]">
          <div className="relative">
            ...
            >
              ...
              {/* Progress fill - uses sepia/gold accent */}
              <div
                className="absolute left-0 top-0 h-full rounded-full transition-all duration-200"
                style={{
                  width: `${progress}%`,
                  background: 'linear-gradient(135deg, var(--book-accent) 0%, color-mix(in srgb, var(--book-accent) 80%, white) 100%)',
                }}
              />
              ...
              {isHovering && (() => {
                ...
                return (
                  ...
                  >
                    ...
                    {year && <span style={{ color: 'var(--book-accent)' }}> • {year}</span>}
                  </div>
                );
              })()}
            </div>

            {/* Decade date markers below progress bar (desktop only) */}
