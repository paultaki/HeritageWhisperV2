...
// Import V4 CSS with premium styling
import "./book-v4.css";
...
function BookV4PageContent() {
  ...
  const [isBookOpen, setIsBookOpen] = useState(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('book-v4-is-open') === 'true';
    }
    return false;
  });
  const [currentSpreadIndex, setCurrentSpreadIndex] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem('book-v4-spread-index');
  ...
  const [currentMobilePage, setCurrentMobilePage] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem('book-v4-mobile-page');
      return saved ? parseInt(saved, 10) : 0;
    }
    return 0;
  });
  ...
  useEffect(() => {
    sessionStorage.setItem('book-v4-is-open', isBookOpen.toString());
  }, [isBookOpen]);

  useEffect(() => {
    sessionStorage.setItem('book-v4-spread-index', currentSpreadIndex.toString());
  }, [currentSpreadIndex]);

  useEffect(() => {
    sessionStorage.setItem('book-v4-mobile-page', currentMobilePage.toString());
  }, [currentMobilePage]);
  ...
  useEffect(() => {
    ...
    if (storyIndex !== -1) {
      ...
      window.history.replaceState({}, '', '/book-v4');
    }
