...
export async function POST(request: NextRequest) {
  try {
    ...
    const counterField = hasSkipCount ? "skip_count" : "shown_count";
    const currentCount = Number(prompt[counterField] ?? 0);
    const nextCount = currentCount + 1;
    const shouldRetire = nextCount >= 3;
    ...
    if (shouldRetire) {
      ...
      const { error: historyError } = await supabaseAdmin.from("prompt_history").insert({
        ...
        shown_count: hasSkipCount ? prompt.shown_count ?? 0 : nextCount, // keep shown_count info if present
        skip_count: hasSkipCount ? nextCount : null,
        outcome: "skipped",
        created_at: prompt.created_at ?? new Date().toISOString(),
      });
      if (historyError) logger.error("Error archiving prompt to history:", historyError);
      ...
      if (deleteError) {
        logger.error("Error deleting retired prompt:", deleteError);
        return NextResponse.json({ error: "Failed to retire prompt" }, { status: 500 });
      }
    } else {
      // Increment counter and mark last_shown_at
      ...
      if (updateError) {
        logger.error("Error updating prompt counter:", updateError);
