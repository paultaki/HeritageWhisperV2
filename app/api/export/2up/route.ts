...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.debug("[Export 2up] Auth successful, user:", userId);

    const { bookId } = await request.json();

    // Generate a temporary print token (valid for 5 minutes)
    const printToken = generatePrintToken(userId);
    logger.debug("[Export 2up] Generated print token for user:", userId);
    ...
    const printUrl = `${baseUrl}/book/print/2up?printToken=${printToken}&userId=${userId}`;
