...
export const BookPage = React.forwardRef<HTMLDivElement, BookPageProps>(
  ({ story, pageNum, onScroll, position, allStories = [], onNavigateToStory, fontSize = 18, isOwnAccount = true, viewMode = 'chronological' }, ref) => {
      ...
      return stories.map((storyItem, idx) => {
        ...
        return (
          <div key={storyItem.id}>
            {showHeader && (
              <h3 className="text-lg font-bold tracking-tight text-stone-700 mt-5 mb-1.5 first:mt-0 font-serif">
            ...
            <button
              ...
              className="flex justify-between items-baseline w-full hover:bg-amber-50 px-2 py-1.5 rounded transition-colors cursor-pointer text-left"
            >
              <span className="text-stone-600 flex-1 pr-3 hover:text-amber-800 text-base font-serif">
                {storyItem.title}
              </span>
              <span className="text-stone-400 text-sm whitespace-nowrap book-v3-date">
                {storyItem.storyYear}
              </span>
            </button>
          </div>
    ...
    // Premium cream paper background color
    const paperBg = "#fdfbf7";
    const paperBgDark = "#f8f4eb";
            ...
            <div
              ...
              style={{
                backgroundImage: position === "left"
                  ? `radial-gradient(160% 85% at 110% 50%, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0) 55%),
                     radial-gradient(120% 60% at -10% 50%, rgba(0,0,0,0.06) 0%, rgba(0,0,0,0) 58%)`
                  : `radial-gradient(160% 85% at -10% 50%, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0) 55%),
                     radial-gradient(120% 60% at 110% 50%, rgba(0,0,0,0.06) 0%, rgba(0,0,0,0) 58%)`,
              }}
            ></div>

            <div className="relative h-full w-full p-[24px] md:p-[28px] lg:p-[32px]">
              <div className="h-full w-full rounded-[10px] ring-1 backdrop-blur-[0.5px] ring-amber-900/5 flex items-center justify-center"
                style={{ background: `${paperBg}cc` }}
              >
                <div className="text-center space-y-8 p-8 w-full flex flex-col items-center">
                  <h1
                    className="text-5xl font-serif mb-4 text-center"
                    style={{ fontFamily: "Crimson Text, Georgia, serif", color: "#3d2e1f" }}
                  >
                    Family Memories
                  </h1>
                  <div className="w-24 h-0.5" style={{ background: "linear-gradient(90deg, transparent, #CBA46A, transparent)" }}></div>
                  <p className="text-lg leading-relaxed max-w-md text-center italic font-serif" style={{ color: "#6b635a" }}>
                    A collection of cherished moments, stories, and lessons from a life well-lived.
                  </p>
                  <p className="text-base mt-8 text-center font-serif" style={{ color: "#8b7355" }}>
                    These pages hold the precious memories that shaped our family&apos;s journey.
                  </p>
                </div>
              </div>
              <div className="absolute bottom-3 left-0 right-0 flex justify-between px-8 text-sm pointer-events-none z-20" style={{ color: "#8b7355" }}>
    ...
    // Handle endpaper page - premium marbled pattern
    if (story === 'endpaper') {
      return (
        <div ref={pageRef} className={`absolute inset-y-0 ${position === "left" ? "left-0" : "right-0"} w-1/2 [transform-style:preserve-3d]`}>
          ...
          >
            ...
            <div
              ...
              style={{
                backgroundImage: `url("data:image/svg+xml,%3Csvg width='400' height='400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='marble'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.015' numOctaves='8' seed='2' /%3E%3CfeColorMatrix type='saturate' values='0.2'/%3E%3C/filter%3E%3Crect width='400' height='400' filter='url(%23marble)' fill='%23B8A080'/%3E%3C/svg%3E")`,
            ...
            <div
              ...
              style={{
                backgroundImage: `radial-gradient(ellipse at 20% 30%, rgba(139,107,74,0.25) 0%, transparent 50%),
                                  radial-gradient(ellipse at 80% 70%, rgba(139,107,74,0.2) 0%, transparent 50%),
                                  radial-gradient(ellipse at 40% 80%, rgba(139,107,74,0.15) 0%, transparent 40%)`,
              }}
            ></div>
            ...
            {/* Inner gutter shadow */}
            <div className={`absolute inset-y-0 ${position === "left" ? "right-0" : "left-0"} w-10 pointer-events-none bg-gradient-to-${position === "left" ? "l" : "r"} to-transparent from-black/10 via-black/5`}></div>

            {/* Page number */}
            <div className="absolute bottom-3 left-0 right-0 flex justify-between px-8 text-[11px] pointer-events-none z-20" style={{ color: "#8b7355" }}>
      ...
      return (
        <div ref={pageRef} className={`absolute inset-y-0 ${position === "left" ? "left-0" : "right-0"} w-1/2 [transform-style:preserve-3d]`}>
          ...
          >
            ...
            {/* Gutter shadow */}
            <div className={`absolute inset-y-0 ${position === "left" ? "right-0" : "left-0"} w-10 pointer-events-none z-10 bg-gradient-to-${position === "left" ? "l" : "r"} to-transparent from-black/10 via-black/5`}></div>

            <div className="relative h-full w-full p-[24px] md:p-[28px] lg:p-[32px]" style={{ pointerEvents: 'auto' }}>
              <div className="h-full w-full rounded-[10px] ring-1 backdrop-blur-[0.5px] ring-amber-900/5" style={{ background: `${paperBg}cc`, pointerEvents: 'auto' }}>
                <div
                  ref={ref}
                  onScroll={(e) => {
                    onScroll(e);
                  ...
                  className="h-full w-full rounded-[8px] outline-none p-5 overflow-y-auto book-v3-scroll"
                  style={{
                ...
                >
                  <h1 className="text-5xl font-semibold text-center mb-8 font-serif" style={{ color: "#3d2e1f" }}>
                    Table of Contents
                  </h1>
      ...
      return (
        ...
        >
          <div
            ...
            style={{ background: paperBg, pointerEvents: 'auto' }}
          >
            {/* Paper texture */}
            <div
              className="absolute inset-0 pointer-events-none z-10 rounded-[16px]"
        ...
        <div ref={pageRef} className={`absolute inset-y-0 ${position === "left" ? "left-0" : "right-0"} w-1/2 [transform-style:preserve-3d]`}>
          ...
          >
            ...
            <div className="relative h-full w-full p-[24px] md:p-[28px] lg:p-[32px] z-10">
              <div className={`absolute inset-y-0 ${position === "left" ? "right-0" : "left-0"} w-8 pointer-events-none z-[1] bg-gradient-to-${position === "left" ? "l" : "r"} to-transparent from-black/8 via-black/4`}></div>

              <div className="relative h-full w-full rounded-[10px] ring-1 backdrop-blur-[0.5px] ring-amber-900/5 overflow-hidden z-10"
                style={{ background: `${paperBg}cc` }}
              >
                <DecadeIntroPage
                  decade={story.decade}
    ...
    return (
      ...
      >
        {/* Main page */}
        <div
          className={`relative h-full w-full rounded-[16px] ring-1 shadow-2xl [transform:rotateY(${position === "left" ? "3deg" : "-3deg"})_translateZ(0.001px)] ring-black/15 book-v3-page-stack ${position === "left" ? "book-v3-page-stack-left" : "book-v3-page-stack-right"}`}
          style={{
            maxWidth: '100%',
            pointerEvents: 'auto',
            background: paperBg
        ...
        >
          ...
          {/* Spine shadow */}
          <div className={`book-v3-spine-shadow-${position}`}></div>
          ...
          <div
            ...
            style={{
              zIndex: 2,
              backgroundImage: `repeating-linear-gradient(${position === "left" ? "90deg" : "270deg"}, rgba(139,107,74,0.08) 0px, rgba(139,107,74,0.08) 1px, transparent 1px, transparent 2px)`,
              opacity: 0.35
            }}
          ></div>

          <div className="relative h-full w-full p-[24px] md:p-[28px] lg:p-[32px]" style={{ zIndex: 10 }}>
            <div className="h-full w-full rounded-[10px] ring-1 backdrop-blur-[0.5px] ring-amber-900/5 overflow-hidden" 
              style={{ position: 'relative', zIndex: 15, background: `${paperBg}cc` }}
            >
              <div
                ref={ref}
                onScroll={(e) => {
              ...
              {scrollState.hasScroll && !scrollState.hasUserScrolled && (
                <div className="book-v3-fade-bottom"></div>
              )}
...
// Add Next Story Card - Premium heirloom style CTA for empty pages
function AddNextStoryCard() {
  ...
  return (
    <div
      ...
      style={{
        background: "#faf6ee",
        border: "2px dashed rgba(203, 164, 106, 0.4)",
        boxShadow: isHovered
          ? "0 12px 32px rgba(139, 107, 74, 0.15)"
          : "0 6px 20px rgba(139, 107, 74, 0.08)",
    ...
    >
      ...
      <div
        ...
        style={{
          background: "linear-gradient(to bottom right, #f5efe3, #ebe4d4)",
          borderBottom: "1px dashed rgba(203, 164, 106, 0.3)",
        }}
      >
        ...
        <div className="mb-3 relative">
          ...
          <div
            ...
            style={{
              background: "radial-gradient(circle, #CBA46A 0%, transparent 70%)",
              opacity: isHovered ? 1 : 0.5,
              transition: "opacity 0.3s ease",
            }}
          />
        </div>
        <p className="text-sm px-6 text-center font-serif italic" style={{ color: "#8b7355" }}>
      ...
      <div className="px-5 py-5 space-y-4">
        ...
        <p
          ...
          style={{ color: "#6b635a", maxWidth: "260px", lineHeight: 1.5 }}
        >
          Every memory you share becomes a treasured page in your family&apos;s legacy
        </p>

        {/* CTA Button - Gold gradient matching the premium theme */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            handleClick();
          ...
          style={{
            background: "linear-gradient(135deg, #CBA46A 0%, #B8935A 100%)",
            boxShadow: isHovered
              ? "0 8px 20px rgba(203, 164, 106, 0.45)"
              : "0 4px 12px rgba(203, 164, 106, 0.3)",
        ...
        {/* Helper Text */}
        <p className="text-xs text-center" style={{ color: "#a89f94" }}>
...
// Story Content Component - Premium styling
function StoryContent({ story, position, pageNum, fontSize = 18, isOwnAccount = true }: { story: Story; position: "left" | "right"; pageNum: number; fontSize?: number; isOwnAccount?: boolean }) {
  ...
  const getPhotoRotation = () => {
    if (!story.id) return 'book-v3-photo-rotate-1';
    const hash = story.id.charCodeAt(0) % 5;
    return `book-v3-photo-rotate-${hash + 1}`;
  };
  ...
  return (
    <>
      {/* Edit button - Subtle pencil icon in corner */}
      {isOwnAccount && typeof story === 'object' && 'id' in story && (
        <button
          onClick={() =>
            router.push(
              `/review/book-style?edit=${story.id}&returnPath=${encodeURIComponent(`/book-v3?storyId=${story.id}`)}`,
            )
          }
          className="book-v3-edit-button hidden md:flex"
      ...
      {photos.length > 0 && (() => {
        ...
        return (
          <div className="mb-5 mx-auto" style={{ maxWidth: "85%" }}>
            <div
              className={`relative overflow-hidden cursor-pointer group book-v3-photo ${getPhotoRotation()}`}
            ...
            {currentPhoto?.caption && (
              <p className="text-[12px] mt-2 italic" style={{ color: "#6b635a" }}>
                {currentPhoto.caption}
              </p>
            )}
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="book-v3-date">
          {story.lifeAge !== undefined && `Age ${story.lifeAge} · `}
          {story.storyYear}
        </div>
      </div>

      {/* Audio Player - Minimalist gold/brown theme */}
      {story.audioUrl && (
        <div className="book-v3-audio mb-4">
          <button
            onClick={toggleAudio}
          ...
          <div className="book-v3-audio-time">
      ...
      <div className="space-y-4 font-serif book-v3-content">
        {story.transcription?.split("\n\n").map((paragraph, i) => (
          <p
            key={i}
            className={i === 0 && !photos.length ? 'book-v3-drop-cap' : ''}
            style={{
              fontFamily: "Crimson Text, Georgia, serif",
      ...
      {/* Wisdom clip - Handwritten note style */}
      {story.wisdomClipText && (
        <div className="book-v3-wisdom">
